...
namespace SaiModel
{
	...
	public:
		...
		std::vector<LinkMassParams> getAllLinkMassParamsLocal() const;
		std::vector<LinkMassParams> getAllLinkMassParamsGlobal() const;
		...
		*/
		 ...
		 * world frame, and the internal forces and moments.
		 *
		...
		vector<Vector3d> getInertialCenters();

		vector<Matrix3d> getInertiaMatrices();

		std::vector<ContactModel> &environmentalContacts()
		{
			return _environmental_contacts;
		}

		RigidBodyDynamics::Model *rbdlModel() const
		{
			return _rbdl_model;
		}

		void setRbdlModel(RigidBodyDynamics::Model *model)
		{
			_rbdl_model = model;
		}
