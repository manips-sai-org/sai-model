...
int main(int argc, char** argv) {
	...
	SaiModel::SaiModel* robot_default = new SaiModel::SaiModel(robot_fname);
	SaiModel::SaiModel* robot = new SaiModel::SaiModel(robot_fname);
	SaiModel::SaiModel* robot_with_load = new SaiModel::SaiModel(robot_load_fname);
	int dof = robot->dof();
	...
	robot->addLoad(ee_link, mass, com, inertia, Affine3d::Identity(), "load");
	...
	for (int i = 0; i < n_samples; ++i) {
		...
		VectorXd dq = VectorXd::Random(dof);
		robot->setQ(q);
		robot->setDq(dq);
		robot->updateModel();
		robot_with_load->setQ(q);
		robot_with_load->setDq(dq);
